...
public class Rectangle_RenderBatch2D extends RenderBatch2D {

    public Rectangle_RenderBatch2D(int maxBatchSize, int zIndex) { super(maxBatchSize, zIndex); }
    ...
    public void init() {
        super.init(new BufferLayout(Arrays.asList(
                ...
                new VertexBufferElement(ShaderDataType.Float2, "a_Size"),
                new VertexBufferElement(ShaderDataType.Float, "a_Thickness"),
                new VertexBufferElement(ShaderDataType.Float, "a_Fade"),
                new VertexBufferElement(ShaderDataType.Float, "a_CornerRadius"),
    ...
    @Override
    public void render() {
        Profiler.startTimer("Render in Rectangle_RenderBatch2D");
        boolean rebufferData = false;
        for (int i = 0; i < this.numberOfQuads; i++) {
            if (this.quads[i].getClass() != ShapeRenderer.class)
                continue;

            if (((ShapeRenderer) this.quads[i]).getShapeType() != ShapeRenderer.ShapeType.Rectangle)
                continue;
        ...
        Profiler.stopTimer("Render in Rectangle_RenderBatch2D");
    }
    ...
    protected void loadVertexProperties(int index) {
        ...
        if (shape.getShapeType() != ShapeRenderer.ShapeType.Rectangle)
            return;
        ...
        for (int i = 0; i < 4; i++) { // 4 vertices per sprite
            ...
            vertices[offset + 9]    = shape.getSize().x;
            vertices[offset + 10]   = shape.getSize().y;

            vertices[offset + 11]   = shape.getThickness();
            vertices[offset + 12]   = shape.getFade();

            vertices[offset + 13]   = shape.getCornerRadius();

            // Load entity ID
            vertices[offset + 14]   = shape.gameObject.getIncrementedID() + 1; // We used 0 id for invalid object
